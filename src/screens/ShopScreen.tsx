...
export const ShopScreen: React.FC = () => {
  ...
  const closeCart = () => {
    Animated.timing(cartSheetY, { toValue: 400, duration: 200, useNativeDriver: true }).start(() =>
      setCartOpen(false)
    );
  };
        ...
        <View style={styles.productBottom}>
          ...
          >
            <Ionicons
              ...
              size={18}
              color={item.inStock ? '#fff' : 'rgba(255,255,255,0.4)'}
            />
          </TouchableOpacity>
        </View>
      </View>
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <EmptyState icon="compass-outline" text="No tours yet — add them in Sanity" />
          }
        />
      ) : (
        <FlatList
          ...
          contentContainerStyle={styles.listContent}
          ListEmptyComponent={
            <EmptyState icon="bag-handle-outline" text="No products yet — add them in Sanity" />
          }
        />
      )}
...
const CartSheet: React.FC<{ translateY: Animated.Value; onClose: () => void }> = ({
  translateY,
  onClose,
}) => {
  const { items, totalPrice, setQty, clear } = useCart();
  ...
  return (
    <Animated.View
      style={[styles.cartSheet, { transform: [{ translateY }], paddingBottom: insets.bottom + 16 }]}
    >
          ...
          items.map((item) => (
            <View key={`${item.product.id}::${item.variantId ?? ''}`} style={styles.cartItem}>
              ...
              <View style={styles.cartQtyRow}>
                <TouchableOpacity
                  onPress={() => setQty(item.product.id, item.quantity - 1, item.variantId)}
                  style={styles.qtyBtn}
                >
...
const EmptyState: React.FC<{ icon: keyof typeof Ionicons.glyphMap; text: string }> = ({
  icon,
  text,
}) => (
...
const styles = StyleSheet.create({
  ...
  skeletonImage: { height: 160, backgroundColor: 'rgba(0,0,0,0.05)' },
  skeletonInfo: { padding: 16, gap: 10 },
  skeletonLine: { height: 20, borderRadius: 10, backgroundColor: 'rgba(0,0,0,0.05)' },
});
