    ...
    <View style={styles.card}>
          ...
          <Animated.View style={[styles.qrWrap, { height: qrHeight, overflow: 'hidden' }]}>
            <LinearGradient colors={['#111', '#1C1C2E']} style={styles.qrCard}>
              ...
              <Text style={styles.qrCardSub}>
                ...
                {item.time ? ` · ${item.time}` : ''} · {guestLabel}
              </Text>
      ...
      <View style={styles.cardFooter}>
        ...
        <Text style={styles.siteText}>
          {item.site_id === 'wondersofrome' ? 'Wonders of Rome' : 'Tickets in Rome'}
        </Text>
      </View>
    </View>
  );
...
export const MyTicketsScreen: React.FC = () => {
  ...
  const [, setQrOpenCount] = useState(0);
  ...
  const fetchBookings = useCallback(async () => {
    ...
    if (!trimmed || !supabase) {
      if (!supabase) setError('Supabase not configured.');
      return;
    }
    setLoading(true);
    setError(null);
      ...
      {loading ? (
          ...
          {[1, 2, 3].map((i) => (
            <View key={`skeleton-${i}`} style={styles.skeletonCard}>
              ...
              <View style={styles.skeletonGrid}>
                {[1, 2, 3, 4].map((j) => (
                  <View key={`j-${j}`} style={styles.skeletonPill} />
                ))}
              </View>
            </View>
          ))}
            ...
            searched && !loading ? (
              <View style={styles.emptyWrap}>
                ...
                <Text style={styles.emptySubtitle}>
                  Use the same email from your booking confirmation.
                </Text>
              </View>
            ) : null
          }
        ...
        !loading && (
          <View style={styles.emptyWrap}>
            ...
            <Text style={styles.emptySubtitle}>
              Enter the email you used when booking. Confirmed bookings include a QR entry ticket.
            </Text>
          </View>
        )
      )}
...
const styles = StyleSheet.create({
  ...
  skeletonBadge: { height: 22, width: 80, borderRadius: 11, backgroundColor: 'rgba(0,0,0,0.05)' },
  skeletonGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
  skeletonPill: { height: 28, width: 90, borderRadius: 14, backgroundColor: 'rgba(0,0,0,0.05)' },
});
